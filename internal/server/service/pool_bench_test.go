...
// TestPostgresPoolSpeedup measures the incremental cost of adding a test
// environment to a warm pool (CREATE DATABASE) versus starting a fresh
// Postgres container from scratch. This is a one-shot comparison for PR
// documentation, not a permanent test.
func TestPostgresPoolSpeedup(t *testing.T) {
	if testing.Short() {
	...
	const pgImage = postgresDefaultImage
	pid := os.Getpid()

	// Pre-pull so image pull time doesn't skew results.
	t.Log("pre-pulling", pgImage)
	rc, err := cli.ImagePull(ctx, pgImage, image.PullOptions{})
	if err != nil {
	...
	pool := NewPostgresPool(pid)
	defer pool.Close()

	warmStart := time.Now()
	first, err := pool.Acquire(ctx, pgImage)
	if err != nil {
	...
	incrStart := time.Now()
	second, err := pool.Acquire(ctx, pgImage)
	if err != nil {
	...
	unpooled := &pgBackend{
		image:         pgImage,
		containerName: fmt.Sprintf("rig-bench-unpooled-%d", pid),
	}
	freshStart := time.Now()
